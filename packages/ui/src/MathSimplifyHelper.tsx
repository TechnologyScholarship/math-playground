...
import { distribute, matchingPairParts, matchingPairPattern, numPattern, OpType, replacer, RxMatch, symbolPattern, tidy, unwrapBracketPair } from './latexutil';
...
export function MathSimplifyHelper({ latex, replaceHandler, ...props }: MathSimplifyHandlerProps) {
  ...
  replace(
    constSumPattern,
    ([_, a, op, b]) =>
      `+ ${asNumber(a) + asNumber(b) * (op === '-' ? -1 : 1)}`
  );
  replace(constProductPattern, ([_, a, b]) => `${asNumber(a) * asNumber(b)}`);
  replace(rx`(${numPattern}?\s*(?:${symbolPattern}\s*)*)\\times\s*(${matchingPairPattern})`, ([_, coefficient, term]) => {
    const { begin, end, pair } = matchingPairParts(term);
    return `${begin}\\left(${distribute(OpType.PRODUCT, coefficient, pair.replace(unwrapBracketPair.regexFull(), '$1'))}\\right)${end}`;
  });
